...
bool isTrustedPublicHeader(
    const std::string &path,
    const std::unordered_set<std::string> &trusted_public_headers) {
  return trusted_public_headers.contains(canonicalPath(path));
}
...
DeclarationRole classifyDeclaration(
  ...
  if (declaration.qualified_name == "matcore::mdsl::gemm" &&
      declaration.parameter_types ==
          std::vector<std::string>{"matcore::mdsl::out_arg",
                                   ...
                                   "const matcore::mdsl::matrix_view &",
                                   "matcore::mdsl::policy"}) {
    return isTrustedPublicHeader(declaration.source_file,
                                 trusted_public_headers)
               ? DeclarationRole::gemm
               : DeclarationRole::untrusted_gemm;
  }
...
void collectDeclarations(const JsonValue &node,
                         ...
                         const std::unordered_set<std::string> &trusted_headers,
  ...
  if (const JsonValue *inner = innerArray(node)) {
    for (const JsonValue &child : inner->GetArray()) {
      collectDeclarations(child, namespaces, last_source_file, trusted_headers,
                          result);
    }
  }
}
...
bool AstJsonBootstrapFrontend::processCall(
  ...
  const DeclarationRole callee_role =
      roleForId(callee->declaration_id, declarations);
  if (callee_role != DeclarationRole::gemm &&
      callee_role != DeclarationRole::untrusted_gemm) {
    return true;
  }
  ...
  if (callee_role == DeclarationRole::untrusted_gemm) {
    reject("Matcore operations must resolve to the canonical declaration in "
...
bool AstJsonBootstrapFrontend::extract(const Options &options, Result &result) {
  ...
  std::unordered_set<std::string> trusted_headers;
  for (const std::string &path : options.trusted_public_headers) {
    if (!path.empty()) {
      trusted_headers.insert(canonicalPath(path));
    }
  }
  if (trusted_headers.empty()) {
    result.diagnostics.push_back(
        Diagnostic{.file = display_path,
                   .message = "compiler has no trusted <matcore/mdsl.h> path"});
    return false;
  }
  std::string declaration_source_file = display_path;
  collectDeclarations(document, {}, declaration_source_file, trusted_headers,
                      declarations);
