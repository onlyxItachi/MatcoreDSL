#!/usr/bin/env python3
"""Actual production multi-source execution and pre-link refusal regression."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument("--case", default="core")
    args = parser.parse_args()
    driver = args.driver.resolve(strict=True)
    if args.expected_driver_sha256 and digest(driver) != args.expected_driver_sha256:
        parser.error("driver differs from supplied identity")
    output = args.output.absolute() if args.output else Path(tempfile.mkdtemp(prefix="mdslc-program-tests-")) / "run"
    if output.exists():
    ...
    report = {"schema": "authenticated-multi-source-test-v1", "status": "running",
              ...
              "authority": "production_source_only_single_invocation",
    ...
    if args.case != "core":
        if args.case == "paths":
            cases = [
                ("duplicate_path", "main.cpp", "duplicate source TU physical identity"),
                ("duplicate_symlink", "main.cpp", "duplicate source TU physical identity"),
                ("duplicate_hardlink", "main.cpp", "duplicate source TU physical identity"),
            ]
        elif args.case == "ownership":
            cases = [
                ("candidate_export", "main_candidate.cpp", "original host defines symbol owned by trusted artifact"),
                ("candidate_weak", "main_candidate_weak.cpp", "original host defines symbol owned by trusted artifact"),
                ("forged_header", "main_forged.cpp", "experimental canonical header physical FileID or exact bytes disagree"),
                ("unselected", "main_unselected.cpp", "program contains an unselected region definition"),
            ]
        else:
            parser.error("unknown case group")
    try:
        for name, host, rejection in cases:
            ...
            if name.startswith("duplicate_") and name != "duplicate_main":
                duplicate = fixtures / "first.mdsl"
                if name != "duplicate_path":
                    duplicate = work / "duplicate.mdsl"
                    if name == "duplicate_symlink": duplicate.symlink_to(fixtures / "first.mdsl")
                    else:
                        # The checkout and /tmp can be different filesystems.
                        # Both linked paths must denote the same scratch inode.
                        original = work / "original.mdsl"
                        shutil.copyfile(fixtures / "first.mdsl", original)
                        os.link(original, duplicate)
                        command += ["--host", original]
                command += ["--host", duplicate]
            ...
            if rejection:
                ...
                existing_sha = digest(executable)
                refused = run(command, work)
                if (not rejected_before_link(refused, "output already exists", False) or
                        digest(executable) != existing_sha):
                    raise RuntimeError("program publication overwrote an existing executable")
                record["existing_output_refused_without_clobber"] = True
        ...
        raise
    save()
    print("PASS authenticated multi-source", len(report["cases"]), "cases")
